...
search_and_replace()
{
    ...
    str=$(grep -w "$LHS\s*=" $SECURITYCONFIGFILE)
    ...
    sed -i -e "s?\(\s*{*\)\(${LHS}\)\(\s*= *\)\(\"*\)\(${OLD_RHS}\)\(\"*\)\(;\)?\1\2\3\4${RHS}\6\7?g" $SECURITYCONFIGFILE
}


LINE=$1
SECURITYCONFIGFILE=$2

if [ "$SECURITYCONFIGFILE" = "" ]
then
    SECURITYCONFIGFILE="/srv/openair5G/targets/bin/security_config_rebaca.conf"
fi
